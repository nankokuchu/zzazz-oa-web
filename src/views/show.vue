<template>
  <div class="container">
    ...
    <div class="notice" v-if="taskId !== 0">
      ...
      <p>{{ process.title }}</p>
      <span class="pass">{{ process.status === 1 ? '承認中' : process.status === 2 ? '承認済' : '否認' }}</span>
    </div>
    <div class="footer" v-if="isApprove">
      ...
      <div class="right-button">
        <van-button @click="approve(-1)" type="default" size="small">否認</van-button>
        <span style="margin: 0 4px"></span>
        <van-button @click="approve(1)" type="info" size="small">承認</van-button>
      </div>
    </div>
  </div>
</template>
